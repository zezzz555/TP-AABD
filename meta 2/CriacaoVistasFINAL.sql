...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "LEIAABDG10"."VIEW_J_A2018018279" ("CONCELHO", "ID_TRANSPORTE", "ARMAZEM_ORIGEM", "CENTRO_DESTINO", "DATA") AS 
  select centros_vacinacao.concelho,
        transportes.id_transporte,
        armazens.morada||' '||armazens.distrito as Armazem_Origem,
        centros_vacinacao.morada||' '||centros_vacinacao.concelho as Centro_Destino,
        transportes.data
from centros_vacinacao,transportes,armazens
where centros_vacinacao.id_centro=transportes.id_centro
and transportes.id_armazem=armazens.id_armazem
group by centros_vacinacao.concelho, transportes.id_transporte,armazens.morada||' '||armazens.distrito,centros_vacinacao.morada||' '||centros_vacinacao.concelho, transportes.data
order by centros_vacinacao.concelho asc, transportes.data desc
;
--------------------------------------------------------
--  DDL for View VIEW_J_A2018018386
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "LEIAABDG10"."VIEW_J_A2018018386" ("NOME", "REGIAO", "NTURNOS_REALIZADOS", "TEMPOMEDIO_ADMINISTRACAO", "NPESSOAS_VACINOU") AS 
...
;
--------------------------------------------------------
--  DDL for View VIEW_K_A2018018279
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "LEIAABDG10"."VIEW_K_A2018018279" ("FASE", "SETOR", "LOCAL", "CONCELHO", "NUM_REJEICOES", "ID_CONVOCATORIA", "NUM_CONVOCADOS", "PERCENT_REJEICAO") AS 
  select fases.numero_fase as Fase,
        locais_de_trabalho.tipo as Setor,
        locais_de_trabalho.local_trabalho as Local,
        pessoas.concelho,
        a.Nrej as Num_rejeicoes,
        convocatorias.id_convocatoria,
        count(convocatorias.id_convocatoria) as Num_convocados,
        ((a.Nrej/count(convocatorias.id_convocatoria))*100) as Percent_rejeicao
from fases,convocatorias,pessoas,trabalham,locais_de_trabalho,(select fases.id_fase,convocatorias.id_convocatoria,pessoas.concelho as C,count(convocatorias.id_convocatoria) as Nrej 
                                                                from fases,pessoas,convocatorias,locais_de_trabalho,trabalham
                                                                where fases.id_fase=convocatorias.id_fase 
                                                                and convocatorias.cc=pessoas.cc 
                                                                and pessoas.cc=trabalham.cc 
                                                                and trabalham.id_localprof=locais_de_trabalho.id_localprof
                                                                and lower(locais_de_trabalho.tipo)='publico'
                                                                and convocatorias.resposta='n'
                                                                group by fases.id_fase, convocatorias.id_convocatoria, pessoas.concelho) a
where convocatorias.id_fase=fases.id_fase
and convocatorias.cc=pessoas.cc
and pessoas.cc=trabalham.cc
and trabalham.id_localprof=locais_de_trabalho.id_localprof
and lower(locais_de_trabalho.tipo)='publico' and convocatorias.id_convocatoria=a.id_convocatoria
group by fases.numero_fase, pessoas.concelho, locais_de_trabalho.tipo, locais_de_trabalho.local_trabalho, a.Nrej, 
convocatorias.id_convocatoria
;
--------------------------------------------------------
--  DDL for View VIEW_K_A2018018386
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "LEIAABDG10"."VIEW_K_A2018018386" ("NOMEUTENTE", "NDOSE", "DATATOMA", "DATA_DA_DOSE", "VALIDADEFRASCO", "DATAPRODUCAOLOTE", "LABVACINA") AS 
